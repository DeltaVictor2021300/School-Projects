        ...
        <div class="col-md-8">
            ...
            if (!is_numeric($albumId) || $albumId <= 0) {
                ...
                if (!empty($pictures)) {
                    ...
                    $pictureTitle = $pictures[$currentPictureIndex]['Title'];
                    echo "<h2 id='mainPictureName' class='text-center' style='margin: 0 0 10px;'>" . ($pictureTitle ? $pictureTitle : 'NO TITLE') . "</h2>";
            ...
            if (!empty($pictures)) {
                ...
                if (empty($comments)) {
                    ...
                    foreach ($comments as $comment) {
                        $authorName = htmlspecialchars(getNameById($comment['Author_Id']), ENT_QUOTES, 'UTF-8');
                        $commentText = htmlspecialchars($comment['Comment_Text'], ENT_QUOTES, 'UTF-8');
                        echo "<p class='comment-author'><span style='color: blue; font-style: italic; font-weight: bold;'>{$authorName}:&nbsp;</span>{$commentText}</p>";
                    }
                }
                echo "</div>";
                echo "</div>";
