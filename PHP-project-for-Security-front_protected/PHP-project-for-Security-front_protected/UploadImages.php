        ...
        if ($files['error'][$key] == 0) {
            if ($filePath) {
                ...
                if ($imageDetails && in_array($imageDetails[2], $supportedImageTypes)) {
                    ...
                    $pdo = getPDO();
                    $stmt = $MyPdo->prepare("INSERT INTO picture (Album_Id, File_Name, Title, Description) VALUES (:albumId, :filename, :title, :description)");
                    $stmt->execute([
                    ...
                    $error = "One or more uploaded files are not supported types";
                    unlink($filePath);
                }
            } else {
